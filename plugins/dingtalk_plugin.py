...
class EchoMarkdownHandler(dingtalk_stream.ChatbotHandler):
    ...
    async def process(self, callback: dingtalk_stream.CallbackMessage):
        ...
        if text.startswith("/google"):
            ...
            text_blocks = SerpAPIPlugin.search(query)
            result = SerpAPIPlugin.parse_text_blocks(text_blocks)
            self.reply_markdown(query, result, self.incoming_message)
        else:
    ...
    async def receive_incoming_messages(self, deck) -> None:
        if not self.started:
            ...
            self.client.register_callback_handler(
                dingtalk_stream.chatbot.ChatbotMessage.TOPIC, self.handler
            )
